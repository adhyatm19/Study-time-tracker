...
export function useFloatingTimer(snapshot: FloatingTimerSnapshot) {
  ...
  useEffect(() => {
    ...
    if (snapshot.status !== "idle" && message === "Start a timer before opening the floating timer.") {
      setMessage(null);
    }
    ...
    updateFloatingWindow();
  }, [closeFloatingTimer, getActiveWindow, message, snapshot, updateFloatingWindow]);
